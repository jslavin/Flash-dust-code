...
! at least part of the interval/circle/sphere and is marked for refinement.
           ...
           if (dist2 <= radius**2) then
              ...
              if (lrefine(b) < lref ) then
                 refine(b)   = .true.
                 derefine(b) = .false.
                 !write(*,'("marked to refine: lrefine(",i5,") =",' &
                 !    // '" lref =",i5)') b,lref
                 nrefined = nrefined + 1
                 ...
                 refine(b) = .true.
                 derefine(b) = .false.  ! added by JDS 4/23/2021
                 !write(*,'("marked to refine: lref = 0")')
                 nrefined = nrefined + 1
              ! Added 4/26/2021 - allows us to derefine if we want
              else if (lrefine(b) > lref) then
                 !write(*,'("marked to derefine: lrefine(",i5,") =",' &
                 !    // '" lref =",i5)') b,lref
                 refine(b) = .false.
                 derefine(b) = .true.
              endif
     ...
     !write(*,'("number of refined blocks = ",i5)') nrefined 
