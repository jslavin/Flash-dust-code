...
! from the WORK array, and performs a prolongation operation
...
! direction.  `Y' interpolations follow, but use the interpolated
...
! the array interp_mask_work must have data in it that is >= 0.
...
! then work variable 1 will be prolongated using simple direct injection, work variable 2
...
!
! It is applied to WORK variables whose corresponding element
! of interp_mask_work is set to below 20.  Usually there is at most one
