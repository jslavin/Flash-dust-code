...
!!  lostElements=0
  ...
  !if(lostParticle) write(*,'("particle lost, tag = ",F6.0," x =",ES12.4)') &
  !    dataElement(TAG_PART_PROP),dataElement(gr_ptPosx)
  ...
  if(.not.lostParticle) then
     ...
     blk=dataElement(gr_ptBlk)
     lostParticle=((blk==NONEXISTENT).or.(blk==LOST))
     !if(lostParticle) write(*,'("particle lost, tag = ",F4.0," y =",ES12.4)') &
     !    dataElement(TAG_PART_PROP),dataElement(gr_ptPosy)
  end if
  ...
  if(.not.lostParticle) then
     ...
     blk=dataElement(gr_ptBlk)
     lostParticle=((blk==NONEXISTENT).or.(blk==LOST))
     !if(lostParticle) write(*,'("particle lost, tag = ",F4.0," z =",ES12.4)') &
     !    dataElement(TAG_PART_PROP),dataElement(gr_ptPosz)
  end if
