...
#ifdef XRAY_ATTRCOUNT
  call RuntimeParameters_get("xray_maxXrayCount",maxPerProc)
  propCount=max(XRAY_ATTRCOUNT,propCount)                       ! DANGER
  gr_ptMaxPerProc = max(maxPerProc,gr_ptMaxPerProc)             ! OK -> 2nd buffer dimension
#endif

#ifdef TRAY_ATTRCOUNT
  call RuntimeParameters_get("thsc_maxRayCount",maxPerProc)
  propCount=max(TRAY_ATTRCOUNT,propCount)                       ! DANGER
  gr_ptMaxPerProc = max(maxPerProc,gr_ptMaxPerProc)             ! OK -> 2nd buffer dimension
#endif
...
#ifdef XRAY_ATTRCOUNT
  if (XRAY_ATTRCOUNT /= propCount) then
      call Logfile_stamp (XRAY_ATTRCOUNT,"[gr_ptInit]: Value of XRAY_ATTRCOUNT is = ")
      call Logfile_stamp (propCount,"[gr_ptInit]: Value of propCount is = ")
      call Logfile_stampMessage ("Different 1st dimensions for Destination and Source Buffer!")
      call Driver_abortFlash ("[gr_ptInit]: XRAY_ATTRCOUNT must match propCount (see Logfile).")
  endif
#endif

#ifdef TRAY_ATTRCOUNT
  if (TRAY_ATTRCOUNT /= propCount) then
      call Logfile_stamp (TRAY_ATTRCOUNT,"[gr_ptInit]: Value of TRAY_ATTRCOUNT is = ")
      call Logfile_stamp (propCount,"[gr_ptInit]: Value of propCount is = ")
      call Logfile_stampMessage ("Different 1st dimensions for Destination and Source Buffer!")
      call Driver_abortFlash ("[gr_ptInit]: TRAY_ATTRCOUNT must match propCount (see Logfile).")
  endif
#endif
