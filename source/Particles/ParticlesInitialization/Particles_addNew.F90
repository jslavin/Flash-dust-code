...
#include "Flash_mpi.h"
  ...
  integer :: i, tagOffset, ierr, effCount, k
...
!-------------------------------------------------------------------------------
  ...
  call MPI_AllReduce(doLocal(1),doGlobal(1),2,MPI_LOGICAL,MPI_LAND, &
      pt_meshComm,ierr)
  doAll = doGlobal(1)
  ...
  if(doAny) then
     ...
     if(present(pos)) then
        ...
        do i = 1,effCount
            ...
            call InitSingleParticle(k, xr, yr, zr, vxr, vyr, vzr, &
                sim_pmass, sim_pdens) !, blockId)
        enddo
        pt_numLocal = pt_numLocal + effCount
        call Grid_moveParticles(particles,NPART_PROPS,pt_maxPerProc,&
             pt_numLocal,pt_indexList,pt_indexCount,coords_in_blk)
