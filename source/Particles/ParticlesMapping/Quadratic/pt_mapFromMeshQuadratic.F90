     ...
     if (pt_geometry == CARTESIAN) then
        ...
        do i = 1,numAttrib
           ...
           E = (solnVec(meshIndex,ip  ,jp-1,kp  ) &
                -2.*solnVec(meshIndex,ip  ,jp  ,kp  ) &
                +solnVec(meshIndex,ip  ,jp+1,kp  )) * 0.5*deltaCellInverseY**2
           ! Added 9/11/2019 by JDS to prevent undershoot or overshoot in
           ! interpolation
           attrmin = min(solnVec(meshIndex,ip,jp-1,kp), &
               solnVec(meshIndex,ip,jp+1,kp), &
               solnVec(meshIndex,ip-1,jp,kp), solnVec(meshIndex,ip+1,jp,kp), &
               solnVec(meshIndex,ip,jp,kp))
           attrmax = max(solnVec(meshIndex,ip,jp-1,kp), &
               solnVec(meshIndex,ip,jp+1,kp), &
               solnVec(meshIndex,ip-1,jp,kp), solnVec(meshIndex,ip+1,jp,kp), &
               solnVec(meshIndex,ip,jp,kp))
           
           partAttribVec(i) = min(max((A + (B + C*hx)*hx + (D + E*hy)*hy), &
               attrmin), attrmax)
        end do
     endif
