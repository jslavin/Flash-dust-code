...
!!
!!***
!! Modified 27 May 2016 by Jonathan Slavin - cl_logGam replaces cl_nH (latter
!! was a constant = 1) in order to use a heating term
...
subroutine Cool_init()
  ...
  use Cool_data, ONLY :cl_fctn_file,cl_Nmax,cl_logT, cl_ne, &
       ...
       cl_logGam, cl_dlogGdlogU, cl_dtmin, cl_smallt
...
!==============================================================================
  ...
  if(useCool) then
     ...
     call readSdTable (cl_fctn_file(1:len_trim(cl_fctn_file)), &
          cl_logT, cl_ne, cl_nt, cl_logGam, cl_logLnet, cl_logLnorm, &
     ...
     do i = 1, cl_N-1
        ...
        cl_dlogGdlogU(i) = (cl_logGam(i+1) - cl_logGam(i)) * dlogUinv
