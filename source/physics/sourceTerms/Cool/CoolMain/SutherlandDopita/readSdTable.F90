...
!                                   density in cm^-3
!               logGam(:)         Array returning heating function Gamma
!                                   in log(erg s^-1)
...
!
! Modified 5/27/2016 by JDS - replaced nH (which was a constant = 1) with 
! logGam the heating rate per nucleon (erg/s)
! Removed:
!               nH(:)             Array returning hydrogen number density
!                                   in cm^-3

subroutine readSdTable (file, logT, ne, nt, logGam, logLnet, logLnorm, &
...
!===============================================================================
  ...
  real, intent(INOUT), dimension(Nmax) :: logT, ne, nt, logGam, logLnet, &
      logLnorm, logU, logtau, P12, rho24, Ci, mubar
  ...
  !if (associated(logGam))   deallocate (logGam)
  ...
  do i = 1, N
     read (1,*) logT(i), ne(i), nt(i), logGam(i), logLnet(i), logLnorm(i), &
